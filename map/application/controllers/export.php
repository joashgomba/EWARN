...
class Export extends CI_Controller {
   ...
   function exportlist()
   {
	   ...
	   $enddate = $reportingperiod_two->to;
	    ...
	    $forms = $this->exportmodel->get_export_records($reg_id,$dist_id,$hf_id,$from,$to,$reporting_year,$reporting_year2);
	   ...
	   $districtselect = '<select name="district_id" id="district_id" onChange="GetHealthFacilities(this)">';
		...
		if(empty($districts))
		{
		   ...
		   elseif($level==1)
		   {
			   ...
			   $districtdata = $this->districtsmodel->get_by_zone($zone_id);
			   foreach($districtdata as $dkey => $districtdatum)
			   {
