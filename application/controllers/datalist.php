...
class Datalist extends CI_Controller {
   ...
   public function index()
   {
	 ...
	 $level = $this->erkanaauth->getField('level');
	  ...
	  if(getRole() != 'SuperAdmin' &&  $level !=2 && $level !=1 && $level !=4 && $level !=6)
	  {
	   ...
	   //if(empty($reporting_year) || empty($reporting_year2) || empty($from) || empty($to) || empty($district_id)){
		...
		else
		{
			...
			else
			{
				...
				if($level==2 || $level==6)//regional
				{
					$lists = $this->reportingformsmodel->get_full_list($reporting_year,$reporting_year2,$from,$to,$dist_id,$reg_id,$zn_id,$hf_id);
				}
				if($level==1)//zonal
				{
					$lists = $this->reportingformsmodel->get_full_list_level($reporting_year,$reporting_year2,$from,$to,$dist_id,$reg_id,$zn_id,$hf_id,$level);
				}
				...
				if($gender==1)
				{
					...
					$table .= '<thead>';
					$table .= '<tr border><th>Week</th><th>HFC Name</th><th>SARI <5yr</th><th >SARI >5yr</th><th >ILI <5yr</th><th >ILI >5yr</th><th >AWD <5yr</th><th >AWD >5yr</th><th >BD <5yr</th><th >BD >5yr</th><th >OAD <5yr</th><th>OAD >5yr</th><th>Diph <5yr</th><th>Diph >5yr</th><th >WC <5yr</th><th >WC >5yr</th><th >Meas <5yr</th><th >Meas >5yr</th><th >NNT</th><th >AFP <5yr</th><th >AFP >yr</th><th >AJS</th><th>VHF</th><th>Mal >5yr</th><th >Mal <5yr</th><th >Men <5yr</th><th >Men >5yr</th><th>UnDis</th><th >UnDis</th><th >OC</th><th>SRE</th><th>Pf</th>
					...
					foreach ($lists as $key => $list) {
						...
						$diphofivetot = $list->diphofivemale + $list->diphofivefemale;
						$wctot = $list->wcmale + $list->wcfemale;
						$wcofivetot = $list->wcofivemale + $list->wcofivefemale;
						$meastot = $list->measmale + $list->measfemale;
						$measofivetot = $list->measofivemale + $list->measofivefemale;
						$nnttot = $list->nntmale + $list->nntfemale;
						$afptot = $list->afpmale + $list->afpfemale;
						$afpofivetot =  $list->afpofivemale + $list->afpofivefemale;
						...
						$menofivetot = $list->suspectedmenegitisofivemale + $list->suspectedmenegitisofivefemale;
						...
						else
						{
							...
							$table .= '<td '.$diphbg.'>'.$diphtot.'</td><td '.$diphofivebg.'>'.$diphofivetot.'</td>';
							
							$table .= '<td '.$wcbg.'>'.$wctot.'</td><td '.$wcofivebg.'>'.$wcofivetot.'</td>';
							
							$table .= '<td '.$measbg.'>'.$meastot.'</td><td '.$measofivebg.'>'.$measofivetot.'</td>';
							
							$table .= '<td '.$nntbg.'>'.$nnttot.'</td><td '.$afpbg.'>'.$afptot.'</td>';
							
							$table .= '<td '.$afpofivebg.'>'.$afpofivetot.'</td><td '.$ajsbg.'>'.$ajstot.'</td>';
							
							$table .= '<td '.$vhfbg.'>'.$vhftot.'</td><td '.$malubg.'>'.$malutot.'</td>';
							
							$table .= '<td '.$malobg.'>'.$malotot.'</td><td '.$menbg.'>'.$mentot.'</td><td '.$menofivebg.'>'.$menofivetot.'</td>';
				...
				else if($gender==2)
				{
					...
					foreach ($lists as $key => $list) {
						...
						$oadotot = $list->oadofivemale + $list->oadofivefemale;
						$oadtot = $oadotot + $oadutot;
						$diphtot = $list->diphmale + $list->diphfemale + $list->diphofivemale + $list->diphofivefemale;
						$wctot = $list->wcmale + $list->wcfemale + $list->wcofivemale + $list->wcofivefemale;
						$meastot = $list->measmale + $list->measfemale + $list->measofivemale + $list->measofivefemale;
						$nnttot = $list->nntmale + $list->nntfemale;
						$afptot = $list->afpmale + $list->afpfemale + $list->afpofivemale + $list->afpofivefemale;
						$ajstot = $list->ajsmale + $list->ajsfemale;
						$vhftot = $list->vhfmale + $list->vhffemale;
						$malutot = $list->malufivemale + $list->malufivefemale;
						$malotot = $list->malofivemale+$list->malofivefemale;
						$mentot = $list->suspectedmenegitismale + $list->suspectedmenegitisfemale + $list->suspectedmenegitisofivemale + $list->suspectedmenegitisofivefemale;
						$undistot = $list->undismale + $list->undisfemale;
						$undistwotot = $list->undismaletwo + $list->undisfemaletwo;
						$octot = $list->ocmale + $list->ocfemale;
		  ...
		  if($level==4)//National
		 {
		   ...
		   if(empty($healthfacility_id))
				...
				if($gender==1)
				{
					$table .= '<table id="listtable">';
					$table .= '<thead>';
					$table .= '<tr border><th>Week</th><th>HFC Name</th><th>SARI <5yr</th><th >SARI >5yr</th><th >ILI <5yr</th><th >ILI >5yr</th><th >AWD <5yr</th><th >AWD >5yr</th><th >BD <5yr</th><th >BD >5yr</th><th >OAD <5yr</th><th>OAD >5yr</th><th>Diph <5yr</th><th>Diph >5yr</th><th >WC <5yr</th><th >WC >5yr</th><th >Meas <5yr</th><th >Meas >5yr</th><th >NNT</th><th >AFP <5yr</th><th >AFP >5yr</th><th >AJS</th><th>VHF</th><th>Mal >5yr</th><th >Mal <5yr</th><th >Men <5yr</th><th >Men >5yr</th><th>UnDis</th><th >UnDis</th><th >OC</th><th>SRE</th><th>Pf</th>
					<th>Pv</th><th>Pmix</th><th>Cons</th><th>User</th><th>Validate</th>
					</tr>';
					...
					foreach ($lists as $key => $list) {
						...
						else
						{
							...
							$table .= '<td '.$diphbg.'>'.$diphtot.'</td><td '.$diphofivebg.'>'.$diphofivetot.'</td><td '.$wcbg.'>'.$wctot.'</td><td '.$wcofivebg.'>'.$wcofivetot.'</td>';
							
							$table .= '<td '.$measbg.'>'.$meastot.'</td><td '.$measofivebg.'>'.$measofivetot.'</td><td '.$nntbg.'>'.$nnttot.'</td>';
							
							$table .= '<td '.$afpbg.'>'.$afptot.'</td><td '.$afpofivebg.'>'.$afpofivetot.'</td><td '.$ajsbg.'>'.$ajstot.'</td>';
