...
class Alerts extends CI_Controller {
   ...
   public function alertlist($week_no,$reporting_year)
   {
	   ...
	   else
	   {
		   ...
		   else
		   {
			   ...
			   else
			   {
				  ...
				  $table .= '<tr><td>'.$reporting_period->week_no.'/'.$reporting_period->epdyear.'</td><td>'.$alert['disease_name'].'</td><td>'.$healthfacility->health_facility.'</td><td>'.$district->district.'</td><td>'.$region->region.'</td><td>'.$alert['cases'].'</td><td>'.$alert['deaths'].'</td><td>'.$alert['notes'].'</td><td>'.$verification_status.'</td><td>'.$include_bulletin.'</td><td><a href="'.base_url().'index.php/alerts/edit/'.$alert['id'].'" class="tooltip-success" data-rel="tooltip" title="Edit">
																	<span class="green">
	   ...
	   $this->load->view('alerts/alertslist', $data);
   }
   ...
   public function edit_validate($id)
   {
       ...
       if ($this->form_validation->run() == false) {
		   ...
		   redirect('alerts/alertlist/'.$week_no.'/'.$year_no.'', 'refresh');
       }
   }
