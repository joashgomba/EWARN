		...
		else
		{
			...
			else
			{
			   ...
			   endforeach;
                ...
                //$lists = $this->formsmodel->get_full_list($reporting_year,$reporting_year2,$period_one,$period_two,$dist_id,$reg_id,$zn_id,$hf_id,$diseasearray);

                $lists = $this->formsmodel->get_full_list_array($reporting_year,$reporting_year2,$epicalendaridArray,$dist_id,$reg_id,$zn_id,$hf_id,$diseasearray);
				$table = '   <style>
				#listtable
				{
				...
				if($gender==1)
				{
					...
					else
					{
						...
						foreach ($lists as $key => $list) {
							...
							$table .= '<td>'.$list->week_no.'/'.$list->reporting_year.'</td><td>'.$list->health_facility.'</td>';
							...
							foreach($diseasearray as $key=>$diseasecode):
								...
								if($diseasecode=='SARI')
                                {
                                    $under_number = $list->SARI_T_U_F;
                                    $over_number = $list->SARI_T_O_F;
                                }
                                if($diseasecode=='ILI')
                                {
                                    $under_number = $list->ILI_T_U_F;
                                    $over_number = $list->ILI_T_O_F;
                                }
                                if($diseasecode=='AWD')
                                {
                                    $under_number = $list->AWD_T_U_F;
                                    $over_number = $list->AWD_T_O_F;
                                }
                                if($diseasecode=='BD')
                                {
                                    $under_number = $list->BD_T_U_F;
                                    $over_number = $list->BD_T_O_F;
                                }
                                if($diseasecode=='OAD')
                                {
                                    $under_number = $list->OAD_T_U_F;
                                    $over_number = $list->OAD_T_O_F;
                                }
                                if($diseasecode=='Diph')
                                {
                                    $under_number = $list->Diph_T_U_F;
                                    $over_number = $list->Diph_T_O_F;
                                }
                                if($diseasecode=='WC')
                                {
                                    $under_number = $list->WC_T_U_F;
                                    $over_number = $list->WC_T_O_F;
                                }
                                if($diseasecode=='Meas')
                                {
                                    $under_number = $list->Meas_T_U_F;
                                    $over_number = $list->Meas_T_O_F;
                                }
                                if($diseasecode=='NNT')
                                {
                                    $under_number = $list->NNT_T_U_F;
                                    $over_number = $list->NNT_T_O_F;
                                }
                                if($diseasecode=='AFP')
                                {
                                    $under_number = $list->AFP_T_U_F;
                                    $over_number = $list->AFP_T_O_F;
                                }
                                if($diseasecode=='AJS')
                                {
                                    $under_number = $list->AJS_T_U_F;
                                    $over_number = $list->AJS_T_O_F;
                                }
                                if($diseasecode=='VHF')
                                {
                                    $under_number = $list->VHF_T_U_F;
                                    $over_number = $list->VHF_T_O_F;
                                }
                                if($diseasecode=='Mal')
                                {
                                    $under_number = $list->Mal_T_U_F;
                                    $over_number = $list->MAL_T_O_F;
                                }

                                if($diseasecode=='Men')
                                {
                                    $under_number = $list->Men_T_U_F;
                                    $over_number = $list->Men_T_O_F;
                                }

                                **/


                                $table .= '<td '.$tdcolor.'>'.$list->$under_element.'</td>';

								$table .= '<td '.$tdcolor.'>'.$list->$over_element.'</td>';

                                /**
                                $table .= '<td '.$tdcolor.'>'.$under_number.'</td>';
                                $table .= '<td '.$tdcolor.'>'.$over_number.'</td>';
                                 * **/
							endforeach;
							
							$undisOne = ($list->undismale + $list->undisfemale);
							$undistwo = ($list->undismaletwo + $list->undisfemaletwo);
							$oc = ($list->ocmale+$list->ocfemale);
							
							if($list->UnDis_total_alerts>0)
							{
							...
							$table .= '<td '.$undisdcolor.'>'.$undisOne.'</td><td '.$undisdcolor.'>'.$undistwo.'</td><td>'.$oc.'</td><td '.$srtdcolor.'>'.$list->sre.'</td><td '.$pftdcolor.'>'.$list->pf.'</td><td>'.$list->pv.'</td><td>'.$list->pmix.'</td><td>'.$list->total_consultations.'</td>';
							
							$table .= '<td>'.$list->username.'</td>';
								...
								if($level==2)//region
								{
									if($list->approved_zone==0)
									{
										if($list->approved_region==0)
										{
											...
											**/
											$table .= '<td><a href="javascript:void(0)" onclick="validateEntry('.$list->id.')" class="btn btn-success radius-4">Validate</a>
<br><a href="javascript:void(0)" onclick="invalidateEntry('.$list->id.')" class="btn btn-danger radius-4">Invalidate</a>
</td>';
										}
										else
										{
					...
					else
					{
						...
						foreach ($lists as $key => $list) {
							...
							foreach($diseasearray as $key=>$diseasecode):
								...
								$table .= '<td '.$tdcolor.'>'.$list->$male_under_five.'</td>';
								$table .= '<td '.$tdcolor.'>'.$list->$female_under_five.'</td>';
								$table .= '<td '.$tdcolor.'>'.$list->$male_over_five.'</td>';
								$table .= '<td '.$tdcolor.'>'.$list->$female_over_five.'</td>';
	   ...
	   //query based on submitted values
		 ...
		 //echo $sql;
               ...
               //$lists = $this->formsmodel->get_full_list_export($reporting_year,$reporting_year2,$period_one,$period_two,$dist_id,$reg_id,$zn_id,$hf_id);
                $lists = $this->formsmodel->get_full_list_epi_export($reporting_year,$reporting_year2,$epicalendaridArray,$dist_id,$reg_id,$zn_id,$hf_id);
				$table = '   <style>
				#listtable
				{
				font-family:"Trebuchet MS", Arial, Helvetica, sans-serif;
  ...
  function gethealthfacilities()
   ...
   public function exportlist()
   {
       ...
       foreach($epilists as $key=>$epilist)
       {
		...
		//$lists = $this->formsmodel->get_full_export_list($reporting_year,$reporting_year2,$period_one,$period_two,$dist_id,$reg_id,$zn_id,$hf_id,$diseasearray);

       $lists = $this->formsmodel->get_full_export_epi_list($reporting_year,$reporting_year2,$epicalendaridArray,$dist_id,$reg_id,$zn_id,$hf_id,$diseasearray);
	   //query based on submitted values
	...
	$table .= '<thead>';
	  ...
	  else
	  {
			...
			foreach ($lists as $key => $list):
				...
				$table .= '<td>'.$list->reporting_year.'</td><td>'.$list->week_no.'</td><td>'.$zone->zone.'</td><td>'.$region->region.'</td><td>'.$district->district.'</td><td>'.$list->health_facility.'</td><td>'.$list->health_facility.'</td><td>'.$list->health_facility_type.'</td><td>'.$list->hf_code.'</td>';
